...
/* mg.jeong SYSFS */
static ssize_t mg_jeong_show(struct kobject *kobj,
		struct kobj_attribute *attr, const char *buf, size_t n)
{
	//pr_err("%s: gpio52 value %x.\n", __func__, gpio_get_value(52));
	//pr_err("%s: gpio16 value %x.\n", __func__, gpio_get_value(16));
    //pr_err("[PM_DEBUG]Before PADCONF_GPMC_NCS1=GPIO52:0x%08x\n", (u32) omap_readw(0x480020B0));
	prm_set_mod_reg_bits(0x1<<2|0x1<<3, // TEDCHO
			OMAP3430_GR_MOD,
			OMAP3_PRM_VOLTCTRL_OFFSET);

	pad_save(0x0004/*GPIO-IN&PULLDOWN*/, 0x48002A18);//  SYS_OFF_MODE
	gpio_set_value(9, 0);		//omap2_pm_dump(2,0,0);

	return 1; //No action
}
static ssize_t mg_jeong_store(struct kobject *kobj,
		struct kobj_attribute *attr, const char *buf, size_t n)
{
	//change volt
	int gpio_no=0;
	...
	if ((sscanf(buf, "%d,%d", &gpio_no, &value) != 2)) {
		pr_err("%s: Invalid input value %d %d.\nInput 'gpio_number value'\n", __func__, gpio_no, value);
		return -EINVAL;
	}
	pr_err("%s: Valid input value gpio_num=%d setting_value=%d.\n", __func__, gpio_no, value);
	pr_err("[PM_DEBUG]Before GPIO_%d set value:0x%d\n", gpio_no, (u32) gpio_get_value(gpio_no));
	gpio_set_value(gpio_no, value);//GPIO16=BT_EN,52=BT_WAKEUP
	pr_err("[PM_DEBUG]After GPIO_%d set value:0x%d\n", gpio_no, (u32) gpio_get_value(gpio_no));
...
static struct kobj_attribute mgjeong =
	__ATTR(mgjeong, 0644, mg_jeong_show, mg_jeong_store);
...
static int __init omap3_pm_init(void)
{
	...
	if (sysfs_create_file(power_kobj, &mgjeong.attr))
		pr_warning("mgjeong: sysfs_create_file failed\n");
